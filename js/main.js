...
if (window.addEventListener) {
    window.addEventListener('load', function () {
        ...
        let toolCurrent;
        let tool_default = 'pencil';
        // active symmetry
        let symmetry;
        let symmetry_default = 'horizontal_vertical';
        // cursor parameters
        let cursorsize;
        let cursorsize_default = 20;
        // active colors
        ...
        let color_cycle_active = false;
        let color_cycle_timer;
        ...
        function init() {
            ...
            let container = canvas_img.parentNode;
            ...
            let tool_select = document.getElementById('dtool');
            if (!tool_select) {
            ...
            {
                toolCurrent = tools.get(tool_default);
                tool_select.value = tool_default;
            }
            // get the symmetry select input
            const symmetry_select = document.getElementById('symmetry');
            if (!symmetry_select) {
            ...
            const forecolor_select = document.getElementById('forecolorpicker');
            if (!forecolor_select) {
            ...
            const backcolor_select = document.getElementById('backcolorpicker');
            if (!backcolor_select) {
            ...
            const cursorsize_select = document.getElementById('cursorsizerange');
        ...
        function ev_cursorsize_change(ev) {
        ...
        tools.set("rectangle", {
            ...
            mousemove(ev) {
                ...
                if (this.x0 == this.xCurr && this.y0 == this.yCurr)
                    return;
                var x = Math.min(this.xCurr, this.x0), y = Math.min(this.yCurr, this.y0), w = Math.abs(this.xCurr - this.x0), h = Math.abs(this.yCurr - this.y0), xs = Math.min(canvas_draw.width - this.xCurr, canvas_draw.width - this.x0), ys = Math.min(canvas_draw.height - this.yCurr, canvas_draw.height - this.y0);
                context_draw.clearRect(0, 0, canvas_draw.width, canvas_draw.height);
                if (!w || !h)
                    return;
                for (let i = -cursorsize / 2; i <= cursorsize / 2; i++) {
        ...
        tools.set("disc", {
            ...
            mousemove(ev) {
                ...
                if (this.x0 == this.xCurr && this.y0 == this.yCurr)
                    return;
                context_draw.clearRect(0, 0, canvas_draw.width, canvas_draw.height);
                const radius = Math.sqrt((this.x0 - this.xCurr) * (this.x0 - this.xCurr) + (this.y0 - this.yCurr) * (this.y0 - this.yCurr));
                for (let i = -cursorsize / 2; i <= cursorsize / 2; i++) {
                    // mouse position
                    context_draw.beginPath();
                    context_draw.arc(this.x0 + i, this.y0 + i, radius, 0, 2 * Math.PI, false);
