...
// conversion HEX <-> RGB
export function pixelHexToRGB(hexColorStr: string, opacity: number = -1): Array<number>
{
...
export function pixelRGBToHex(rgb: [number, number, number])
{
    return "#" + ((1 << 24) + (rgb[0] << 16) + (rgb[1] << 8) + rgb[2]).toString(16).slice(1);
}

// returns true if pixel (x,y) of image data <pix> with width <w> has color array [r,g,b]
export function pixelHasColor(pix: Uint8ClampedArray, w: number, x: number, y: number, rgb: Array<number>)
{
...
export function pixelHasHexColor(pix: Uint8ClampedArray, w: number, x: number, y: number, hexColorStr: string)
{
    return pixelHasColor(pix, w, x, y, pixelHexToRGB(hexColorStr));
}

// get color of pixel (x,y) of image data <pix> with width <w>
export function pixelGetColor(pix: Uint8ClampedArray, w: number, x: number, y: number)
{
    return [ pix[4*(x + w*y)], pix[4*(x + w*y)+1], pix[4*(x + w*y)+2] ];
}

// set color array [r,g,b] to pixel (x,y) of image data <pix> with width <w>
export function pixelSetColor(pix: Uint8ClampedArray, w: number, x: number, y: number, rgb: Array<number>)
{
    pix[4*(x + w*y)+0] = rgb[0];
    pix[4*(x + w*y)+1] = rgb[1];
    pix[4*(x + w*y)+2] = rgb[2];
...
export function pixelSetHexColor(pix: Uint8ClampedArray, w: number, x: number, y: number, colorStr: string): void
{
